...
description: Test specialist for Contract-First TDD. Responsible for test writing, mock generation, and quality gates. Use proactively for test writing tasks.
...
#    ✅ /path/to/worktree/phase-1-auth/tests/api/auth.test.ts
```
...
워크트리에서 작업을 시작합니다.
   - 대상 파일: backend/tests/auth.test.ts
...
기술 스택:
- Vitest (백엔드/프론트엔드 유닛 테스트)
- @hono/node-server + supertest (HTTP 테스트)
...
백엔드 테스트 고려사항:
- Bun 테스트 런타임 사용
- 테스트용 데이터베이스 분리 (Supabase test project)
