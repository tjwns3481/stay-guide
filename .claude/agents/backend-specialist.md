...
description: Backend specialist for Hono + Bun server-side logic, API endpoints, and Supabase access. Use proactively for backend tasks.
...
워크트리에서 작업을 시작합니다.
   ...
   - 테스트: backend/tests/auth.test.ts
```
...
[ ] bun test 실행 시 FAILED?

# T*.1/T*.2 (구현) 커밋 전:
[ ] 기존 테스트 파일 존재? (T0.5.x에서 작성됨)
[ ] bun test 실행 시 PASSED?
[ ] 새 테스트 파일 추가 안 함?
```
