...
description: Database specialist for Prisma schema, Supabase PostgreSQL, and migrations. Use proactively for database tasks.
...
│    3. 마이그레이션/모델 수정                            │
│    4. npx prisma migrate dev && bun test 재실행        │
│  }                                                      │
│  → GREEN 달성 시 루프 종료                              │
└─────────────────────────────────────────────────────────┘
```
...
**완료 조건:** `npx prisma migrate dev && bun test` 모두 통과 (GREEN)
...
1. **마이그레이션 및 테스트 실행 결과 보고**
   ```
   npx prisma migrate dev 실행 결과: ✅ 성공
   bun test 실행 결과:
   ✅ 5/5 테스트 통과 (GREEN)
   ```
...
2. **완료 상태 요약**
   ...
   - 마이그레이션: 001_init, 002_add_guides
   - 인덱스: idx_guides_user_id, idx_blocks_guide_id
   ```
...
- pgvector 확장 활용 (AI 컨시어지 RAG용)
- JSONB 타입 활용 (블록 콘텐츠 저장)
- Array 타입 활용
...
- Prisma 마이그레이션 (prisma/migrations/)
- Supabase RLS 정책 SQL (supabase/migrations/)
